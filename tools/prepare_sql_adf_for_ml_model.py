...
for run_lbl in training_df['run_lbl'].unique():
    parent_child_nuc_arr = np.zeros((4,4))
    ...
    for df_row in reduced_df.itertuples(index=False):
        ...
        if parent_nuc in ordered_nucs and child_nuc in ordered_nucs:
    ...
    feature_arr = np.array((t_irr, avg_flux_mag, *flux_spec_shape))

    array_list.append(parent_child_nuc_arr)
