...
import testing_case_inp_params_to_dict as tiptd

def make_all_input_files(testing_child_dicts, flux_path_modifier, nuclib, volume, trunc_tolerance, inp_file_folder, filenames):
    '''
    :param: testing_child_dicts (numpy array of dictionaries, with shape len(rel_on_time_factors) x len(flux_norm_factors) x len(flux_files)
    ...
    :param: filenames (numpy array with the same shape and physical dimensions as testing_child_dicts)
    '''
    for testing_dict_idx in np.ndindex(testing_child_dicts.shape):
        if testing_child_dicts[testing_dict_idx] == None:
            continue
        else:
            child_dict = testing_child_dicts[testing_dict_idx]
            ...
            with open(inp_file_folder+"/"+filenames[testing_dict_idx], 'w') as new_inp:
...
def main():
    ...
    num_pulses_list = inputs['num_pulses_list']
    ...
    testing_inp_info = tiptd.make_flux_tirr_combos(rel_on_time_factors, flux_norm_factors, flux_files)
